...
const Show = Component.extend({
  ...
  init(config, parent) {
    ...
    this.template = this.template || require("raw-loader!./show.html");
  ...
  redraw() {
    ...
    this.model.time.getFrame(this.model.time.value, values => {
      if (!values) return;

      _this.list.html("");

      const categories = [];
      this.model.marker.data.noFrameSpace.map(dim => {
        categories.push({ dim,
          key: dim,
          category: this.model.marker.data.source.getConcept(dim).name
        });

        this.model.label.data.response
          .forEach(({ dim: key, domain }) => {
            if (!domain || key == dim) return;
            categories.push({
              key,
              dim: domain,
              isSet: true,
              category: this.model.marker.data.source.getConcept(key).name
            })
        });
      });

      const entitiesValuesObj = this.model.label.data.response.reduce((rez, obj) => {
        rez[obj.dim] = obj;
        return rez;
      }, {});
  ...
  showHideSearch() {
    if (!this._readyOnce || this.parent.getPanelMode() !== "show") return;
