...
const ButtonList = Component.extend({
  ...
  init(config, context) {
    ...
    this.model_binds = {
      "change:marker.select": function(evt) {
        ...
        // if(_this.entitiesSelected_1 !== (_this.model.marker.select.length > 0)) {
        //   _this.scrollToEnd();
        // }
        // _this.entitiesSelected_1 = _this.model.marker.select.length > 0;
      },
  ...
  readyOnce() {
    ...
    // if (button_expand){
    //   this.model.ui.config.dialogs.sidebar = (button_expand === true) ? this.model.ui.config.buttons : button_expand;
    // }
    ...
    const button_list = [].concat(this.model.ui.config.buttons);
  ...
  */
  ...
  scrollToEnd() {
    ...
    if (parent.classed("vzb-portrait") && parent.classed("vzb-small")) {
      if (this.model.marker.select.length > 0) target = this.element.node().scrollWidth;
      this.element.node().scrollLeft = target;
    } else {
      if (this.model.marker.select.length > 0) target = this.element.node().scrollHeight;
  ...
  resize() {
    ...
    if (this.getLayoutProfile() === "small" && this.model.ui.config.presentation) {
  ...
  toggleBubbleTrails() {
    this.model.ui.config.chart.trails = !this.model.ui.config.chart.trails;
    this.setBubbleTrails();
  },
  setBubbleTrails() {
    const trails = (this.model.ui.config.chart || {}).trails;
    ...
    btn.classed(class_hidden, this.model.marker.select.length == 0);
  },
  toggleBubbleLock(id) {
    const active = (this.model.ui.config.chart || {}).lockActive;

    if (this.model.marker.select.length == 0 && !active) return;

    let locked = this.model.ui.config.chart.lockNonSelected;
    const time = this.model.state.time;
    locked = locked ? 0 : time.formatDate(time.value);
    this.model.ui.config.chart.lockNonSelected = locked;
  ...
  setBubbleLock() {
    let locked = (this.model.ui.config.chart || {}).lockNonSelected;
    const active = (this.model.ui.config.chart || {}).lockActive;
    const unavailable = (this.model.ui.config.chart || {}).lockUnavailable || false;
    if (!locked && locked !== 0) return;

    if (locked !== 0 && this.model.marker.select.length === 0 && !active) {
      locked = this.model.ui.config.chart.lockNonSelected = 0;
    }
    ...
    //btn.classed(class_unavailable, this.model.marker.select.length == 0 && !active);
  ...
  toggleInpercent() {
    this.model.ui.config.chart.inpercent = !this.model.ui.config.chart.inpercent;
    this.setInpercent();
  },
  setInpercent() {
    if (typeof ((this.model.ui.config.chart || {}).inpercent) === "undefined") return;
    const id = "inpercent";
    const translator = this.model.locale.getTFunction();
    const btn = this.element.selectAll(".vzb-buttonlist-btn[data-btn='" + id + "']");

    btn.classed(class_active_locked, this.model.ui.config.chart.inpercent);
  },
  togglePresentationMode() {
    this.model.ui.config.presentation = !this.model.ui.config.presentation;
    this.setPresentationMode();
  },
  setPresentationMode() {
    ...
    btn.classed(class_active_locked, this.model.ui.config.presentation);
  },
  toggleFullScreen(id, emulateClick) {
    ...
    const btn = this.element.selectAll(".vzb-buttonlist-btn[data-btn='" + id + "']");
    const fs = !this.model.ui.config.fullscreen;
    ...
    this.model.ui.config.fullscreen = fs;
    const translator = this.model.locale.getTFunction();
    ...
    if (!this.resizeInExitHandler) this.root.model.ui.resizeHandler();
